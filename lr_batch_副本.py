...
def merge_data_in_folder(filepath):
    ...
    for filename in filenames:
        ...
        dfs.append(temp_df.drop(columns='label'))
    ...
    label_df = pd.DataFrame(list(label_dict.items()), columns=['cell_id', 'label'])
    merged_df = pd.merge(merged_df, label_df, how='right', on='cell_id').fillna(0)
...
def main():
    ...
    df.to_csv(f'{SAVE_PATH}/sum_report_GPT4.csv', index=False)
